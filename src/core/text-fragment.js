...
//
// Testing against real pages settled the rules this has to respect:
...
export function fragmentUrl(url, phrase) {
  ...
  // A dash marks a prefix or suffix in this syntax ("text=before-,quote")
  // and encodeURIComponent leaves it alone, so a passage with a stray " - "
  // in it was read as something else and opened at the top.
  return url.split('#')[0] + '#:~:text=' + encodeURIComponent(phrase).replace(/-/g, '%2D');
}
