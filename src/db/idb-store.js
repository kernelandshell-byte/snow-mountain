...
export function createIdbStore(db) {
  ...
  return {
    ...
    // Every page and the whole index, in one transaction. Delete everything
    // used to go through deletePages a page at a time, which on a full
    // archive runs past the five minutes Chrome gives one request and stops
    // the worker part way through, with nothing written to the storage log.
    // Clearing the stores takes the same time whatever is in them, and a
    // single transaction means it either all goes or none of it does.
    async deleteAll() {
      const tx = db.transaction(['pages', 'postings', 'meta'], 'readwrite');
      const pages = tx.objectStore('pages');
      const meta = tx.objectStore('meta');
      const deleted = await req(pages.count());
      const stats = (await req(meta.get('stats'))) || emptyStats;
      await req(pages.clear());
      await req(tx.objectStore('postings').clear());
      await req(meta.put({ ...emptyStats }));
      await txDone(tx);
      return { deleted, bytesFreed: deleted ? stats.totalBytes || 0 : 0 };
    },
    ...
    //
    // `skipPinned` counts only pages that could actually be removed toward
    // the limit. Pinned pages are never evicted and gather at the old end of
    // the archive, so without it a slice can be all pinned pages while the
    // expired ones behind them wait for ever.
    ...
    // page and exclusion rules need the address, and reading whole records
    // including their text would mean loading the entire corpus into memory
    ...
    // A site and everything under it. Walks the domain index's keys only, so
    // no page record is read; the index is ordered by the whole domain, not
    // by its end, which is why this cannot be a range.
    async pageIdsForSite(site) {
      ...
      const out = [];
      await new Promise((resolve, reject) => {
        const request = tx.objectStore('pages').index('domain').openKeyCursor();
        request.onsuccess = () => {
          const cursor = request.result;
          if (!cursor) return resolve();
          const domain = String(cursor.key);
          if (domain === wanted || domain.endsWith('.' + wanted)) out.push(cursor.primaryKey);
          cursor.continue();
        };
        request.onerror = () => reject(request.error);
      });
    ...
    // Every page id, most recently read first, straight off the lastSeen
    // index. What "newest first" has to rank by, without loading a record.
    async pageIdsByRecency() {
      const tx = db.transaction('pages', 'readonly');
      const ids = await req(tx.objectStore('pages').index('lastSeen').getAllKeys());
      return ids.reverse();
    },

    // Every page's normalised address, off the urlKey index: enough to apply
    // an exclusion rule, without reading a single page of text.
    async listPageKeys() {
      const tx = db.transaction('pages', 'readonly');
      const out = [];
      await new Promise((resolve, reject) => {
        const request = tx.objectStore('pages').index('urlKey').openKeyCursor();
        request.onsuccess = () => {
          const cursor = request.result;
          if (!cursor) return resolve();
          out.push({ id: cursor.primaryKey, urlKey: cursor.key });
          cursor.continue();
        };
        request.onerror = () => reject(request.error);
      });
