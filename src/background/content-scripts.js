...
// Only the host of an allowlist entry becomes a match pattern: a path or a
// port has no place in one, and Chrome rejects the whole registration for a
// single malformed pattern.
export const matchFor = (host) =>
  '*://*.' + String(host).split('/')[0].replace(/^\*\./, '').replace(/:\d*$/, '') + '/*';
...
async function doSyncContentScripts() {
  ...
  // Nothing watches anything until setup is finished: that is the screen
  // that says what this does, and a person who closed it half way through
  // has not agreed to it. Granting access on the way through setup must not
  // be enough on its own. What would be watched is still worked out, so
  // settings can say what is missing as well as that setup is.
  const setupPending = !settings.setupComplete;
  if (setupPending) matches = [];
