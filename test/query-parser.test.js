...
import { parseWhen } from '../src/core/query-parser.js';

test('a four digit number is a year, not that many days ago', () => {
  assert.equal(parseWhen('2024'), new Date(2024, 0, 1).getTime());
  assert.equal(parseQuery('budget after:2024').after, new Date(2024, 0, 1).getTime());
});

test('a small number is still days ago', () => {
  const now = Date.UTC(2026, 0, 31);
  assert.equal(parseWhen('30', now), now - 30 * 86400000);
});

test('a date is local midnight, and a month is its first day', () => {
  assert.equal(parseWhen('2026-03-05'), new Date(2026, 2, 5).getTime());
  assert.equal(parseWhen('2026-03'), new Date(2026, 2, 1).getTime());
});
