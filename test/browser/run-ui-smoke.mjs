...
await page.waitForSelector('article.recent-item', { timeout: 4000 });
const recentCount = await page.$$eval('article.recent-item', (nodes) => nodes.length);
check('before anything is typed, it shows what was read lately', recentCount > 0 && recentCount <= 8, recentCount);
check('and how much is saved', /pages? saved/.test(await page.textContent('#meta')), await page.textContent('#meta'));
