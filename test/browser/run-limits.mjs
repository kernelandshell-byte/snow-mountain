...
if (pageAnswer.actual === false) {
  ...
  check('and when it is refused, settings says so rather than leaving it unsaid',
    /not granted this archive persistent storage/i.test(text),
...
check('and settings explains it rather than showing a meter that looks fine',
  /ran out of room/i.test(fullText), fullText.replace(/\s+/g, ' ').slice(0, 200));
check('and says what to do about it',
  /freeing space|lowering the size limit/i.test(fullText), fullText.replace(/\s+/g, ' ').slice(0, 200));
