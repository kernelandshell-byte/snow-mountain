...
const results = await page.evaluate(async () => {
  ...
  // Languages, through the worker: the setting it reads is the one the
  // picker saves, and a different form of a word only reaches the page
  // once that language is on.
  await ask(MSG.PAGE_CONTENT, {
    url: 'https://noticias.example/relatorio',
    title: 'Relatório anual',
    text: 'O relatório reúne toda a informação publicada pela equipe ao longo do ano.',
    capturedAt: Date.now(),
  });
  await ask(MSG.SETTINGS_SET, { stemLanguages: ['en'] });
  const englishOnly = await ask(MSG.SEARCH, { query: 'informações' });
  check('with only English on, another Portuguese form finds nothing', englishOnly.total === 0,
    JSON.stringify(englishOnly.results.map((r) => r.url)));
  await ask(MSG.SETTINGS_SET, { stemLanguages: ['en', 'pt'] });
  const withPortuguese = await ask(MSG.SEARCH, { query: 'informações' });
  check('with Portuguese on, it finds the page, and says which word it matched',
    withPortuguese.total === 1 && JSON.stringify(withPortuguese.expanded).includes('informacao'),
    JSON.stringify({ total: withPortuguese.total, expanded: withPortuguese.expanded }));
  return out;
});
