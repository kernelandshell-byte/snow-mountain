...
await shot('setup-languages', base+'/src/ui/setup/setup.html', {
  full: true, before: async (p) => {
    await p.evaluate(() => {
      for (const section of document.querySelectorAll('section[data-step]')) section.hidden = section.dataset.step !== '4';
    });
  },
});
await shot('setup-languages-narrow', base+'/src/ui/setup/setup.html', {
  width: 380, full: true, before: async (p) => {
    await p.evaluate(() => {
      for (const section of document.querySelectorAll('section[data-step]')) section.hidden = section.dataset.step !== '4';
    });
    await p.fill('#languages input[type="search"]', 'japanese');
  },
});
await ctx.close();
console.log('wrote 18 screenshots to ' + out);
