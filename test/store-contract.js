...
export const contractCases = [
  ...
  {
    name: 'deleting everything empties the archive, the index and the statistics',
    async run(store) {
      await store.putPage(PAGE);
      await store.putPage({ url: 'https://other.example/a', title: 'Short', text: 'one two three' });
      const before = await store.readStats();
      const result = await store.deleteAll();
      assertEqual(result.deleted, 2, 'both deleted');
      assertEqual(result.bytesFreed, before.totalBytes, 'freed what was used');
      const after = await store.readStats();
      assertEqual(after.docCount, 0, 'count reset');
      assertEqual(after.totalBytes, 0, 'bytes reset');
      assertEqual(after.totalTokens, 0, 'tokens reset');
      assertEqual((await store.readTerm('fatigue')).length, 0, 'postings gone');
      assertEqual((await store.readTermsWithPrefix('fat')).length, 0, 'prefix scan finds nothing');
      assertEqual((await store.getPageByUrl(PAGE.url)), null, 'record gone');
      const { id, created } = await store.putPage(PAGE);
      assert(created, 'the same address can be kept again');
      assertEqual((await store.readTerm('fatigue')).length, 1, 'and is searchable');
      assert(id > 0, 'with a fresh id');
      assertEqual((await store.deleteAll()).deleted, 1, 'and deleted again');
      assertEqual((await store.deleteAll()).deleted, 0, 'an empty archive deletes nothing');
    },
  },
  {
  ...
  },
  {
    ...
    async run(store) {
      ...
      assertEqual(meta[0].url, PAGE.url, 'address included, for exclusion rules');
  ...
  {
    name: 'a site means the site and everything under it, and nothing that merely ends the same',
    async run(store) {
      const text = 'A page long enough to be a page, about nothing in particular at all.';
      const apex = await store.putPage({ url: 'https://www.example.com/a', title: 'a', text });
      const sub = await store.putPage({ url: 'https://news.example.com/b', title: 'b', text });
      await store.putPage({ url: 'https://notexample.com/c', title: 'c', text });
      const ids = (await store.pageIdsForSite('example.com')).sort((x, y) => x - y);
      assertEqual(ids.join(','), [apex.id, sub.id].sort((x, y) => x - y).join(','), 'apex and subdomain only');
      assertEqual((await store.pageIdsForSite('www.example.com')).length, 2, 'www. means the same site');
      assertEqual((await store.pageIdsForSite('news.example.com')).join(','), String(sub.id), 'a subdomain alone');
    },
  },
  {
    name: 'every page id, most recently read first',
    async run(store) {
      const text = 'Some text that is long enough to count as a page for this purpose.';
      const old = await store.putPage({ url: 'https://a.example/old', title: 'old', text, lastSeen: 1000 });
      const newest = await store.putPage({ url: 'https://a.example/new', title: 'new', text, lastSeen: 3000 });
      const mid = await store.putPage({ url: 'https://a.example/mid', title: 'mid', text, lastSeen: 2000 });
      assertEqual((await store.pageIdsByRecency()).join(','), [newest.id, mid.id, old.id].join(','), 'newest first');
    },
  },
  {
    name: 'addresses can be listed without the pages they belong to',
    async run(store) {
      const { id } = await store.putPage({ url: 'https://www.example.com/x/?utm_source=a', title: 't', text: 'Enough text here to be stored.' });
      const keys = await store.listPageKeys();
      assertEqual(keys.length, 1, 'one row');
      assertEqual(keys[0].id, id, 'with its id');
      assertEqual(keys[0].urlKey, 'https://example.com/x', 'and its normalised address');
      assert(!('text' in keys[0]), 'and no text');
    },
  },
  {
    name: 'the sweep read can skip pinned pages, counting only what could go',
    async run(store) {
      const text = 'A page of text that is long enough to be kept around for a while.';
      const pinnedIds = [];
      for (let i = 0; i < 5; i++) {
        const { id } = await store.putPage({ url: 'https://p.example/' + i, title: 'p', text, lastSeen: 100 + i, pinned: true });
        pinnedIds.push(id);
      }
      const loose = await store.putPage({ url: 'https://p.example/loose', title: 'l', text, lastSeen: 500 });
      const all = await store.oldestPages(3);
      assertEqual(all.length, 3, 'without the option, pinned pages fill the slice');
      assert(all.every((row) => row.pinned), 'and they are all pinned');
      const evictable = await store.oldestPages(3, null, { skipPinned: true });
      assertEqual(evictable.map((row) => row.id).join(','), String(loose.id), 'with it, the one page that could go');
    },
  },
  {
    name: 'an older visit never moves a page back in time',
    async run(store) {
      const text = 'The same text both times, so the second write is a revisit.';
      await store.putPage({ url: 'https://t.example/a', title: 't', text, lastSeen: 5000 });
      await store.putPage({ url: 'https://t.example/a', title: 't', text, lastSeen: 1000 });
      const [row] = await store.listRecent(1);
      assertEqual(row.lastSeen, 5000, 'lastSeen kept the later date');
    },
  },
];
